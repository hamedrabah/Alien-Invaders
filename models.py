...
class Ship(GImage):
    ...
    def collides(self,bolt):
        """
        Returns: True if the bolt was fired by an alien and collides with this ship
            
        Parameter bolt: The laser bolt to check
        Precondition: bolt is of class Bolt
        """
        x1 = bolt.x - BOLT_WIDTH
        y1 = bolt.y + BOLT_HEIGHT
        x2 = bolt.x + BOLT_WIDTH
        y2 = bolt.y - BOLT_HEIGHT
     #   c1               c3    
     #    -----------------
     #    |               | 
     #    -----------------
     #   c2               c4
        ...
        if (self.contains((x1,y1)) or self.contains((x1,y2)) or self.contains((x2,y1)) or self.contains((x2,y2))):
            return True
...
class Alien(GImage):
    ...
    def __init__(self,k,h,source):
        """
        Initializes an alien GImage with a given x coordinate called k, y coordinate called h, and a source 
        for the image file

        Parameter k: The x coordinate of the alien
        Precondition: k is an int or float 
        Parameter h: The y coordinate of the alien
        Precondition: h is an int or float 
        Parameter source: A string of the image file of the alien
    ...
    def __str__(self):
        """
        String representation of the x and y coordinates of the alien
        """
    
        return ('Alien at x='+str(self.x)+' and y='+str(self.y))
...
class Bolt(GRectangle):
    ...
    # INITIALIZER TO create the bolt
    def __init__(self,k,h, direction, fill):
        """
        Initializes a bolt, which is a GRectangle object

        Parameter k: The x coordinate of the bolt
        Precondition: k is an int or float 
        Parameter h: The y coordinate of the bolt
        Precondition: h is an int or float 
        Parameter direction: Direction bolt moves in, can be either up or down
        Precondition: Must be an int, recommended to use one of BOLT_UP (1) or BOLT_DOWN (-1) 
        """
        self._speed = BOLT_SPEED
        self._direction = direction
        self._fill = fill
        self._velocity = self._speed * self._direction
        super().__init__(x=k,y=h,width=BOLT_WIDTH,height=BOLT_HEIGHT,
                         linecolor='black', velocity = self._velocity, direction=direction, fillcolor=fill)
        
    def __str__(self):
        """
        String representation of the x and y coordinates of the bolt
        """
        return ('Bolt at x='+str(self.x)+' and y='+str(self.y))
        
    # ADD MORE METHODS (PROPERLY SPECIFIED) AS NECESSARY
    def isPlayerBolt(self):
        """
        Checks the direction of a bolt. If it is a positive int, the bolt is a player bolt, 
        if the direction is a negative int, the bolt is a alien bolt. 
        """
